...
let addNoteBtn = document.getElementById('addBtn1')
addNoteBtn.addEventListener('click', ()=>{
    ...
    let editCard = document.querySelector('.post')
    let editIndex = editCard.getAttribute('editIndex') 
    allNotes[editIndex] = addTxt.value;
    localStorage.setItem("notes", JSON.stringify(allNotes))
    addTxt.value = ''
    showNotes()
    let addNoteContainer = document.getElementById('message')
    ...
    for(let i=notesObj.length-1; i>=0; i--){
      ...
      <div class="post">
          ...
          <div class="post__options">
            ...
            <div id="${i}"onclick="editNote(this.id)" class="post__option">
              <span class="material-icons"> update </span>
              <p>Update</p>
            </div>

            <div id="${i}"onclick="deleteNote(this.id)"  class="post__option">
...
const chk = document.getElementById('chk');
 ...
 function editNote(noteIndex){
  ...
  let title = document.getElementById('addTxt1')
  

  title.value = allNotes[noteIndex]
